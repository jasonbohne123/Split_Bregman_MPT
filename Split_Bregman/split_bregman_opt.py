...
def qp(cov, mean, w, d, b, lb, ub, lambda2, approach, tol=10e-6, maxiter=25, verbose=True):
    ...
    if approach == "numerical":
        ...
        res = minimize(objective_val, x0=x0, args=(cov, mean, d, b, lambda2),
                       method='SLSQP', bounds=bounds, constraints=lc, options={'maxiter': maxiter}, tol=tol)
        w = res.x
        status = res.status
    ...
    if approach == "closed-form":
        ...
        w = solve_qp(quad, linear.reshape(len(linear),), A=A, b=constraint,
                     initvals=x0, lb=lb_vec, ub=ub_vec, solver="cvxopt")
        status = "solved"
...
def split_bregman(cov, mean, lambda1, lambda2, lb, ub, beta=None, tol=1e-10, maxiter=100, approach=2, verbose=True):
    ...
    while total_cost > tol:
        ...
        w, status = qp(cov, mean, w, d, b, lb, ub, lambda2,
                       approach, tol, maxiter, verbose)
        ...
        # compute total cost
        total_cost = np.dot(np.dot(w, cov), w)-np.dot(w, mean) + \
            (lambda1*np.sum(np.abs(w))+lambda2*np.sum((w)**2))/2
        if verbose:
